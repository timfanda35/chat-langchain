...
1. Install dependencies: 
   ...
   ```
2. Set environment variable: Copy `.env.example` to `.env` and edit `.env` to put **OPENAI API key** instead of `...`.
   ```
   OPENAI_API_KEY=...
   ```
3. Run command to ingest all PDF under `pdfs` into the vectorstore (only needs to be done once).
   ```
   make ingest
   ```
