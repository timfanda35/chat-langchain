...
1. Clone project
   ```
   git clone -b bear-customize https://github.com/timfanda35/chat-langchain
   ```
2. Install dependencies: 
   ```
   pip install -r requirements.txt
   ```
3. Set environment variable: Copy `.env.example` to `.env` and edit `.env` to put **OPENAI API key** instead of `...`.
   ```
   OPENAI_API_KEY=...
   ```
4. Run command to ingest all PDF under `pdfs` into the vectorstore (only needs to be done once).
   ```
   make ingest
   ```
